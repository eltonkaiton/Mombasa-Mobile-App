...
import ferryRoutes from './routes/ferryRoutes.js'; // import ferry routes
...
import staffRoutes from "./routes/staffRoutes.js"; // ✅ Staff login routes
import ferryCrewRoutes from "./routes/ferryCrewRoutes.js"; // ✅ Ferry Crew routes
...
app.use(
  cors({
    origin: "*", // you can replace with frontend URL if needed
    credentials: true,
  })
);
...
app.use("/api/ferrycrew", ferryCrewRoutes); // ✅ Ferry Crew routes
app.use("/api/ferries", ferryRoutes); // 🔹 NEW ferry routes
...
connectDB()
  .then(() => {
    ...
    server.listen(PORT, "0.0.0.0", () => {
      ...
      console.log(`✅ Ferry Crew routes available at: /api/ferrycrew`);
    });
  })
  .catch((err) => {
    console.error(
